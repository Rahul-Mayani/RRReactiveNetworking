...
+ (void)setHUDType:(DGActivityIndicatorAnimationType )type{
    [self sharedView].hUDType = type;
}

+ (void)setTintColor:(UIColor*)color {
    [self sharedView].tintColor = color;
}

+ (void)setDimAmount:(CGFloat)amount {
    [self sharedView].dimAmount = amount;
}
...
- (instancetype)initWithFrame:(CGRect)frame{
    if((self = [super initWithFrame:frame])){
        ...
        _hUDType = DGActivityIndicatorAnimationTypeBallSpinFadeLoader;
        _tintColor = [UIColor colorWithRed:60 green:60 blue:60 alpha:1.0];
        _dimAmount = 0.7;
...
#pragma clang diagnostic pop
    ...
    //
    //self.stringLabel.center = CGPointMake((CGRectGetWidth(self.hudView.bounds)/2), 36.0f);
...
- (void)positionHUD:(NSNotification*)notification{
    ...
    if(notification){
        ...
        keyboardHeight = keyboardSize.height;// self.visibleKeyboardHeight;
    }
...
- (void)moveToPoint:(CGPoint)newCenter rotateAngle:(CGFloat)angle{
    ...
    // 
    //self.hudView.center = CGPointMake(newCenter.x + self.offsetFromCenter.horizontal, newCenter.y + self.offsetFromCenter.vertical + 20 + ((IS_IPHONE_X && ([[UIDevice currentDevice] orientation] == UIDeviceOrientationPortrait)) ? 24 : 0));
...
-(DGActivityIndicatorView *) createDGActivityIndicatorView {

    DGActivityIndicatorView *indicator = [[DGActivityIndicatorView alloc]initWithType : _hUDType tintColor : _tintColor];
...
- (UIView *)indefiniteAnimatedView {
    if(_indefiniteAnimatedView == nil){
        ...
        switch (self.defaultAnimationType) {
            case SVProgressHUDAnimationTypeNative:
                _indefiniteAnimatedView = [self createActivityIndicatorView];
                break;
            case SVProgressHUDAnimationTypeDGActivity:
                _indefiniteAnimatedView = [self createDGActivityIndicatorView];
                break;
            case SVProgressHUDAnimationTypeFlat:
...
- (UIView*)hudView{
    ...
    // 
    //_hudView.backgroundColor = BackGround_Color;//self.backgroundColorForStyle;
...
- (void)setHUDType:(DGActivityIndicatorAnimationType )type{
    if (!_isInitializing) _hUDType = type;
}

- (void)setTintColor:(UIColor*)color {
    if (!_isInitializing) _tintColor = color;
}

- (void)setDimAmount:(CGFloat)amount {
    if (!_isInitializing) _dimAmount = amount;
}
